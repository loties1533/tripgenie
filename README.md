...
### Partage public
Chaque voyage génère un lien public `/share/:id` — consultable sans compte via une fonction PostgreSQL SECURITY DEFINER (n'expose aucune donnée utilisateur).
...
POST /api/ai/generate
        ...
        │
        └─ 6. Sauvegarde PostgreSQL        ← withUser() + RLS si connecté
```
...
PostgreSQL hébergé sur Supabase — accès via le **driver natif `pg`** (pas d'ORM, pas de Supabase JS). Requêtes SQL paramétrées directement, avec RLS géré côté application.

```sql
users        — Comptes, auth JWT custom (pas Supabase Auth)
...
### RLS « maison » (défense en profondeur)

Supabase est utilisé uniquement comme hébergeur PostgreSQL. Le Row Level Security est géré par nous :

- **Rôle PostgreSQL dédié** `tripgenie_app` — sans `BYPASSRLS`, sans droits DDL
- **Variable de session transaction-locale** `app.current_user_id` posée par `withUser()` via `set_config(..., true)`
- **Fail-closed** : sans utilisateur posé, la base ne renvoie aucune ligne
- **2 barrières** : filtre applicatif `WHERE user_id = $1` **+** RLS PostgreSQL

```typescript
// server/db/pg.ts — withUser() : la pièce centrale du RLS maison
await client.query('BEGIN');
await client.query(`SELECT set_config('app.current_user_id', $1, true)`, [userId]);
const result = await fn(client); // toutes les requêtes ici sont filtrées par le RLS
...
| `pg` (node-postgres) | Driver natif PostgreSQL — SQL paramétré, pool de connexions |
...
| Accès inter-utilisateurs | `WHERE user_id = $1` (filtre applicatif) + RLS PostgreSQL (défense en profondeur) |
...
# Base de données (rôle dédié tripgenie_app, sans BYPASSRLS)
DATABASE_URL=postgresql://tripgenie_app:password@host:5432/postgres
...
│   ├── db/
│   │   └── pg.ts               # Driver pg natif — pool, query(), withUser() (RLS)
│   └── docs/
...
| GET | `/api/trips/share/:id` | 🌐 public | Partage public (SECURITY DEFINER) |
...
**Projet solo — Formation Holberton School — Juin 2026**
